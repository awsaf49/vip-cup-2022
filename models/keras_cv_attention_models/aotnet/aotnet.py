...
def deep_stem(inputs, stem_width, use_half_channel=True, activation="relu", last_strides=1, bn_params={},  first_strides=2, name=None):
    hidden_channel = stem_width // 2 if use_half_channel else stem_width
    nn = conv2d_no_bias(inputs, hidden_channel, 3, strides=first_strides, padding="same", name=name and name + "1_")
...
def aot_stem(inputs, stem_width, type=None, activation="relu", quad_stem_act=False, last_strides=1, bn_params={}, first_strides=2, name=None):
    """stem_type in value `[None, "deep", "deep2", "quad", "tiered"]`."""
    if type == "deep":
        nn = deep_stem(inputs, stem_width, activation=activation, last_strides=last_strides, bn_params=bn_params, first_strides=first_strides, name=name)
    elif type == "deep2":  # RegNetZD8_EVO
        nn = deep_stem(inputs, stem_width, use_half_channel=False, activation=activation, last_strides=last_strides, bn_params=bn_params, first_strides=first_strides, name=name)
    elif type == "quad":
        ...
        nn = conv2d_no_bias(inputs, stem_width, 3, strides=first_strides, padding="same", name=name)
    else:
        nn = conv2d_no_bias(inputs, stem_width, 7, strides=first_strides, padding="same", name=name)
    return nn
...
def AotNet(
    ...
    nn = aot_stem(inputs, stem_width, stem_type, activation, quad_stem_act, last_strides=stem_last_strides, bn_params=bn_params, first_strides=first_strides, name="stem_")
