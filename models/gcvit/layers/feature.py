...
class ReduceSize(tf.keras.layers.Layer):
    def __init__(self, keep_dim=False, first_strides=2, **kwargs):
        super().__init__(**kwargs)
    ...
    def build(self, input_shape):
        ...
        self.reduction = tf.keras.layers.Conv2D(dim_out, kernel_size=3, strides=self.first_strides, padding='valid', use_bias=False,
                                                name='reduction')
