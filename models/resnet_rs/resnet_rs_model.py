...
def STEM(bn_momentum: float, bn_epsilon: float, activation: str, first_strides: int):
    ...
    def apply(inputs):
        ...
        x = Conv2DFixedPadding(
            filters=32, kernel_size=3, strides=first_strides, name="stem_conv_1"
        )(inputs)
...
def ResNetRS(
    ...
    # Build stem
    x = STEM(bn_momentum=bn_momentum, bn_epsilon=bn_epsilon, activation=activation, first_strides=first_strides)(
        img_input
    )
